...
class JsonSerializerUtil {
  ...
  static JsonSerializerUtil::PredicateConditionExpression resolve_predicate_condition(
...
template <typename T, T... S, typename F>
constexpr void JsonSerializerUtil::for_sequence(std::integer_sequence<T, S...>, F&& f) {
  using unpack_t = int[];
  (void)unpack_t{(static_cast<void>(f(std::integral_constant<T, S>{})), 0)..., 0};
}

template <typename T>
void JsonSerializerUtil::key_of_type_exists(const jsonView& value, const std::string& key) {
...
template <typename T>
T JsonSerializerUtil::get_entry_from_json(const jsonView& value, const std::string& key) {
...
/*
template <typename... Args>
auto JsonSerializerUtil::run_function_on_operator_type(const OperatorType op_t, Args... args,
                                                       JsonSerializerUtil::SerializationMode mode) {
